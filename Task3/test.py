...
try:
    params_list = [{'nfeatures': nf, 'nOctaveLayers': nl, 'contrastThreshold': ct, 'edgeThreshold': et, 'sigma': s, 'matchThreshold': mf}
        for nf in [0] # 0 means no limit, usually in 1000s
        for nl in [3]
        for ct in [0.04]
        for et in [10]
        for s in [1.6]
    ...
    # with non rotated training dataset:
    # no rotations: 85% acc, 0  false pos, 3 false neg
    # rotations   : 5% acc,  false pos,  false neg
